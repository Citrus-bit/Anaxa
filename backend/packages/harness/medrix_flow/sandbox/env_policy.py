...
def is_blocked_env_name(name: str) -> bool:
    ...
    upper = name.upper()
    return upper in _BLOCKED_EXACT_NAMES or any(fnmatch.fnmatchcase(upper, pattern) for pattern in _SECRET_NAME_PATTERNS)
...
def build_sandbox_env(injected: dict[str, str] | None = None) -> dict[str, str]:
    ...
    env = {name: value for name, value in os.environ.items() if not is_blocked_env_name(name)}
    if injected:
        env.update(injected)
    return env
