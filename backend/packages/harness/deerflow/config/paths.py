...
class Paths:
    ...
    @property
    def research_base_dir(self) -> Path:
        """Directory shared by Anaxa's optional research SQLite sidecar.

        Existing Anaxa installations keep their databases under
        ``.medrix-flow``.  Prefer that location (or an explicit research home)
        so an upgrade does not strand prior projects; fresh DeerFlow installs
        remain isolated under ``$DEER_FLOW_HOME/anaxa-research``.
        """
        if self._base_dir is not None:
            return self._base_dir
        ...
        candidates = (
            ...
            repo_root / "backend" / ".medrix-flow",
            repo_root / ".medrix-flow",
        )
        for candidate in candidates:
            if candidate is not None and candidate.exists():
                return candidate.resolve()

        if deer_home := os.getenv("DEER_FLOW_HOME"):
            return (Path(deer_home).expanduser() / "anaxa-research").resolve()
        return self.base_dir

    @property
    def runtime_db_file(self) -> Path:
        """Path to the legacy lightweight runtime database.

        DeerFlow's primary run/event persistence is SQLAlchemy-backed, but the
        Anaxa research tools still use the small SQLite runtime repository for
        task metadata.  Keep this path in the shared resolver so those tools
        can run on the same configured state directory without importing the
        old ``medrix_flow`` path module.
        """
        return self.research_base_dir / "runtime.sqlite3"

    @property
    def academic_db_file(self) -> Path:
        """Path to the academic research SQLite database."""
        return self.research_base_dir / "academic.sqlite3"

    @property
    def experiment_db_file(self) -> Path:
        """Path to the experiment lab SQLite database."""
        return self.research_base_dir / "experiment.sqlite3"

    @property
    def research_db_file(self) -> Path:
        """Path to the end-to-end research quest SQLite database."""
        return self.research_base_dir / "research.sqlite3"

    def thread_memory_file(self, thread_id: str, *, user_id: str | None = None) -> Path:
        """Return the compatibility memory file for a thread.

        The pre-DeerFlow Anaxa memory layer stores one ``memory.json`` beside
        each thread directory.  DeerFlow's user-isolated layout is selected
        when ``user_id`` is supplied; omitting it intentionally preserves the
        legacy ``{base_dir}/threads/{thread_id}`` location used by older
        callers and existing installations.
        """
        return self.thread_dir(thread_id, user_id=user_id) / "memory.json"
    ...
    def sandbox_workspace_dir(self, thread_id: str, *, user_id: str | None = None) -> Path:
        """Compatibility alias for :meth:`sandbox_work_dir`.

        Some Anaxa tools used the longer ``sandbox_workspace_dir`` spelling.
        Route it through the DeerFlow implementation so validation and
        user-scoped path selection stay identical.
        """
