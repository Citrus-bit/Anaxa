...
class ResearchConfig(BaseModel):
    """Defaults for Anaxa's staged research and manuscript pipeline."""

    manuscript_model: str | None = Field(default=None, description="Optional model for manuscript section generation; None inherits the current thread model.")
    fast_draft_model: str | None = Field(default=None, description="Optional model for fast manuscript drafts; None falls back to manuscript_model.")
    finalization_model: str | None = Field(default=None, description="Optional model for final manuscript review/finalization; None falls back to manuscript_model.")
    manuscript_section_concurrency: int = Field(default=3, ge=1, le=12, description="Maximum concurrent manuscript section generation calls.")
    default_auto_gates: list[str] = Field(default_factory=list, description="Gate types that research_assistant may auto-approve by default.")
    default_max_stages: int = Field(default=5, ge=1, description="Default lifecycle stages advanced by one run_pipeline call.")
    default_delivery_mode: str = Field(default="final_only", description="Default run_pipeline delivery mode: final_only or fast_draft_first.")
    default_quality_mode: str = Field(default="auto_repair", description="Default quality gate mode: auto_repair, audit_only, or strict_gate.")
    default_quality_repair_budget: int = Field(default=2, ge=0, description="Maximum automatic quality-repair approvals per run_pipeline call.")
...
class AppConfig(BaseModel):
    ...
    def resolve_config_path(cls, config_path: str | None = None) -> Path:
        ...
        if config_path:
            ...
            return path
        elif os.getenv("DEER_FLOW_CONFIG_PATH") or os.getenv("MEDRIX_FLOW_CONFIG_PATH"):
            # Preserve Anaxa's legacy environment variable while preferring
            # DeerFlow's name when both are configured.
            configured_path = os.getenv("DEER_FLOW_CONFIG_PATH") or os.getenv("MEDRIX_FLOW_CONFIG_PATH")
            path = Path(configured_path)
            if not Path.exists(path):
                raise FileNotFoundError(f"Config file specified by environment variable config path not found at {path}")
            return path
        else:
