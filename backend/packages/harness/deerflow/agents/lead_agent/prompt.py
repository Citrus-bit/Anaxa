...
from deerflow.agents.lead_agent.prompt_enhancements import VISUAL_SKILL_NAMES, get_visual_quality_prompt
...
def _has_enabled_visual_skill(
    *,
    ...
    user_id: str | None,
    skill_names: frozenset[str] | None,
) -> bool:
    """Check whether visual prompt guidance is applicable to this agent.

    ``skill_names`` is supplied by the deferred skill-search setup when the
    lead is assembled.  For legacy/direct callers that do not provide it, load
    the enabled catalog through the same user-scoped helper used elsewhere in
    this module.  The explicit ``available_skills`` allowlist is applied in
    both paths so custom agents cannot activate guidance for a disallowed skill.
    """
    try:
        if skill_names is None:
            names = {skill.name for skill in get_enabled_skills_for_config(app_config, user_id=user_id)}
        else:
            names = set(skill_names)
        if available_skills is not None:
            names.intersection_update(available_skills)
        return bool(names & VISUAL_SKILL_NAMES)
    except Exception:
        # Prompt construction should not make a run fail solely because an
        # optional skill store is unavailable.  In that case omit the optional
        # visual guidance and keep the core DeerFlow prompt usable.
        logger.debug("Could not resolve enabled visual skills for prompt", exc_info=True)
        return False
...
def apply_prompt_template(
    ...
    *,
    ...
    # Keep the visual contract out of ordinary prompts.  It is intentionally
    # gated by both the caller's request intent and an enabled/allowed visual
    # skill so the model is not asked to run visual QA for text-only work.
