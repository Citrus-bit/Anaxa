...
def build_middlewares(
    ...
    *,
    ...
    Args:
        ...
        skill_names: Optional enabled skill names from the build-time search
            setup. Supplying them avoids a second storage read when visual
            quality gating is evaluated.
    ...
    # VisualQualityMiddleware is request-scoped.  It is enabled only when the
    # caller explicitly marks the turn as visual and a matching visual skill is
    # both enabled and allowed for this agent/user.  Keep it in the lead chain
    # after the shared tool guards so its reminder sees the final
    # ``present_files`` result while all authorization/sandbox checks remain in
    # force.
    ...
    if visual_output_intent:
        from deerflow.agents.lead_agent.prompt import _has_enabled_visual_skill
        from deerflow.agents.middlewares.visual_quality_middleware import VisualQualityMiddleware

        if _has_enabled_visual_skill(
            app_config=resolved_app_config,
            available_skills=available_skills,
            user_id=user_id,
...
def _assemble_lead_agent(config: RunnableConfig, *, app_config: AppConfig) -> LeadAgentAssembly:
    ...
    visual_output_intent = bool(cfg.get("visual_output_intent", False)) and not bool(is_bootstrap)
    ...
    if is_bootstrap:
        ...
        # Keep the bootstrap call compatible with older tool factories while
        # explicitly disabling visual-only capabilities for this setup run.
