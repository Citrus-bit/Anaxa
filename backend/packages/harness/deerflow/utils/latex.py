...
_UNICODE_MATH_TOKEN_RE = re.compile(rf"(?<![$\\])([A-Za-z0-9]+)([{_SUBSCRIPT_CLASS}{_SUPERSCRIPT_CLASS}]+)")
...
def _ensure_ctex_fontset(source: str) -> str:
    ...
    if documentclass_match is not None and documentclass_match.group("class") in _CTEX_CLASSES:
        ...
        if "fontset=" not in options:
            ...
            source = source[: documentclass_match.start()] + rf"\documentclass[{updated_options}]{{{documentclass_match.group('class')}}}" + source[documentclass_match.end() :]
        return source
...
def audit_pdf_text_extractability(
    ...
    *,
    ...
    if cid_marker_count > max_cid_markers and cid_marker_ratio > max_cid_marker_ratio:
        violations.append(f"PDF text extraction contains too many CID placeholders: {cid_marker_count} markers, ratio {cid_marker_ratio:.3f}.")
