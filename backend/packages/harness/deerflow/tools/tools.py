...
def _has_enabled_visual_skill(
    app_config: AppConfig,
    *,
    available_skills: set[str] | None = None,
    user_id: str | None = None,
) -> bool:
    """Return whether a visual skill is enabled for this agent/user.

    Visual quality tools are deliberately opt-in: a request must carry
    ``visual_output_intent`` *and* the active skill catalog must contain a
    visual skill.  Loading through the prompt helper keeps this check aligned
    with the lead prompt's user-scoped skill resolution and avoids a second,
    subtly different storage implementation here.
    """
    try:
        from deerflow.agents.lead_agent.prompt import get_enabled_skills_for_config
        from deerflow.agents.lead_agent.prompt_enhancements import VISUAL_SKILL_NAMES

        enabled_names = {skill.name for skill in get_enabled_skills_for_config(app_config, user_id=user_id)}
        if available_skills is not None:
            enabled_names.intersection_update(available_skills)
        return bool(enabled_names & VISUAL_SKILL_NAMES)
    except Exception:
        # Tool assembly must remain available when a deployment has a broken
        # optional skill store.  The quality tools are a convenience gate, so
        # fail closed rather than exposing them without the matching skill.
        logger.debug("Could not resolve enabled visual skills", exc_info=True)
        return False
...
def get_available_tools(
    ...
    *,
    ...
    Args:
        ...
        visual_output_intent: Whether the current request is expected to produce
            visual output. Quality/refinement tools are exposed only when this
            is true and an enabled visual skill is available.
        available_skills: Optional skill allowlist for the current agent.
        user_id: Optional user identity used for user-scoped skill resolution.
    ...
    # Visual quality tools are request-scoped. Keeping them out of ordinary
    # runs avoids teaching the model to perform a visual checklist when no
    # visual artifact is being produced, while preserving the Anaxa quality
    # gate for chart/image/PPT requests.
    if visual_output_intent and _has_enabled_visual_skill(
        config,
        available_skills=available_skills,
        user_id=user_id,
    ):
        builtin_tools.extend((visual_quality_check_tool, visual_refinement_check_tool))
        logger.info("Including visual quality tools (visual output intent)")
