...
def _refreshed_compose_command(tmp_root: Path) -> str:
    command = f"""
source '{SCRIPT_PATH}'
PROJECT_ROOT='{tmp_root}'
...
def test_refresh_compose_cmd_uses_root_dotenv_for_interpolation():
    """Compose model interpolation must read the checkout's root .env."""
    with tempfile.TemporaryDirectory() as tmpdir:
        tmp_root = Path(tmpdir)
        (tmp_root / ".env").write_text("BETTER_AUTH_SECRET=test-secret\n", encoding="utf-8")

        command = _refreshed_compose_command(tmp_root)

        assert "--env-file ../.env" in command


def test_refresh_compose_cmd_omits_missing_dotenv():
    """Read-only commands must still be constructible in a fresh checkout."""
    with tempfile.TemporaryDirectory() as tmpdir:
        command = _refreshed_compose_command(Path(tmpdir))

        assert "--env-file" not in command


def test_start_refreshes_compose_command_after_creating_dotenv():
    """start should pass the .env it just seeded to Compose interpolation."""
    with tempfile.TemporaryDirectory() as tmpdir:
        tmp_root = Path(tmpdir)
        marker = tmp_root / "compose-args.txt"
        ...
        (tmp_root / "extensions_config.json").write_text("{}\n", encoding="utf-8")
...
""",
        ...
        assert marker.read_text(encoding="utf-8").splitlines()[:3] == [
            "compose",
            "--env-file",
            "../.env",
        ]
