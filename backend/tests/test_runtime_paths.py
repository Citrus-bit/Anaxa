...
def test_extensions_config_accepts_legacy_anaxa_env_var(tmp_path: Path, monkeypatch):
    _clear_path_env(monkeypatch)
    config_path = tmp_path / "legacy-extensions.json"
...
def test_extensions_config_new_env_var_precedes_legacy_alias(tmp_path: Path, monkeypatch):
    _clear_path_env(monkeypatch)
    deerflow_path = tmp_path / "deerflow-extensions.json"
    legacy_path = tmp_path / "legacy-extensions.json"
    deerflow_path.write_text('{"mcpServers": {}, "skills": {}}', encoding="utf-8")
    legacy_path.write_text('{"mcpServers": {}, "skills": {}}', encoding="utf-8")
    monkeypatch.setenv("DEER_FLOW_EXTENSIONS_CONFIG_PATH", str(deerflow_path))
    monkeypatch.setenv("MEDRIX_FLOW_EXTENSIONS_CONFIG_PATH", str(legacy_path))

    assert ExtensionsConfig.resolve_config_path() == deerflow_path


def test_extensions_config_missing_legacy_env_file_raises(tmp_path: Path, monkeypatch):
    _clear_path_env(monkeypatch)
    missing = tmp_path / "missing-legacy-extensions.json"
    monkeypatch.setenv("MEDRIX_FLOW_EXTENSIONS_CONFIG_PATH", str(missing))

    with pytest.raises(FileNotFoundError, match="MEDRIX_FLOW_EXTENSIONS_CONFIG_PATH"):
        ExtensionsConfig.resolve_config_path()
