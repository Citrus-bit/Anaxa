...
def _fake_middleware_class(name: str):
    """Build an isolated constructor stub for one middleware import.

    The ordering guard intentionally keys on middleware class identity.  A
    single shared fake class would make unrelated patched imports look like
    the same short-circuiting middleware and produce a false ordering error.
    """
...
def _stub_runtime_middleware_imports(monkeypatch: pytest.MonkeyPatch) -> None:
    thread_data = _fake_middleware_class("FakeThreadDataMiddleware")
    sandbox = _fake_middleware_class("FakeSandboxMiddleware")
    dangling = _fake_middleware_class("FakeDanglingToolCallMiddleware")
    sandbox_audit = _fake_middleware_class("FakeSandboxAuditMiddleware")
    ...
    monkeypatch.setitem(
        ...
        _module("deerflow.agents.middlewares.thread_data_middleware", ThreadDataMiddleware=thread_data),
    )
    monkeypatch.setitem(
        sys.modules,
        "deerflow.sandbox.middleware",
        _module("deerflow.sandbox.middleware", SandboxMiddleware=sandbox),
    )
    monkeypatch.setitem(
        sys.modules,
        "deerflow.agents.middlewares.dangling_tool_call_middleware",
        _module("deerflow.agents.middlewares.dangling_tool_call_middleware", DanglingToolCallMiddleware=dangling),
    )
    monkeypatch.setitem(
        sys.modules,
        "deerflow.agents.middlewares.sandbox_audit_middleware",
        _module("deerflow.agents.middlewares.sandbox_audit_middleware", SandboxAuditMiddleware=sandbox_audit),
    )
...
def test_build_subagent_runtime_middlewares_threads_app_config_to_llm_middleware(monkeypatch: pytest.MonkeyPatch):
    ...
    dangling = _fake_middleware_class("FakeDanglingToolCallMiddleware")
    sandbox_audit = _fake_middleware_class("FakeSandboxAuditMiddleware")
    input_sanitization = _fake_middleware_class("FakeInputSanitizationMiddleware")
    ...
    monkeypatch.setitem(
        ...
        _module(
            "deerflow.agents.middlewares.input_sanitization_middleware",
            InputSanitizationMiddleware=input_sanitization,
    ...
    assert isinstance(middlewares[0], input_sanitization)  # InputSanitizationMiddleware stub
