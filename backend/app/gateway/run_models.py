...
class RunCreateRequest(BaseModel):
    ...
    # Legacy Anaxa clients used this field to register an externally-owned
    # run.  DeerFlow's canonical stateless/thread endpoints reject it
    # explicitly; the hidden ``legacy_runs`` router consumes it during a
    # rolling upgrade so persisted run history remains addressable.
    run_id: str | None = Field(default=None, description="Legacy external run identifier")
