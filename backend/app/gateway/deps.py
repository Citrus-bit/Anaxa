...
from .research_compat import legacy_research_runtime
...
async def langgraph_runtime(app: FastAPI, startup_config: AppConfig) -> AsyncGenerator[None, None]:
    ...
    *startup-time* run-events configuration the underlying ``event_store``
    ...
    async with AsyncExitStack() as stack:
        ...
        # Anaxa's academic/experiment/research APIs use their own SQLite
        # schemas.  Keep them in a sidecar context so DeerFlow's SQLAlchemy
        # runtime remains authoritative and can still run with memory-only
        # persistence or without the optional scientific dependencies.
        await stack.enter_async_context(legacy_research_runtime(app))
...
def get_run_service(request: Request) -> Any:
    """Return the pre-DeerFlow Anaxa run service when a caller opts in.

    DeerFlow's active lifecycle is exposed through the module-level service
    functions in ``app.gateway.services``.  The optional hidden migration
    router still serves clients that explicitly install ``app.state.run_service``
    (and therefore must not silently construct a second runtime).
    """
    value = getattr(request.app.state, "run_service", None)
    if value is None:
        raise HTTPException(status_code=503, detail="Legacy run service not available")
    return value


def _require_legacy_service(attr: str, label: str) -> Callable[[Request], Any]:
    """Resolve one optional Anaxa service, returning a precise 503 when absent."""

    def dep(request: Request) -> Any:
        value = getattr(request.app.state, attr, None)
        if value is None:
            runtime = getattr(request.app.state, "research_runtime", None)
            value = getattr(runtime, attr, None) if runtime is not None else None
        if value is None:
            raise HTTPException(status_code=503, detail=f"{label} not available")
        return value
...
# Optional Anaxa research services.  These are deliberately separate from the
# DeerFlow RunContext accessors above so a missing scientific extra cannot
# affect ordinary chat, runs, or Gateway startup.
get_academic_service = _require_legacy_service("academic_service", "Academic research service")
get_experiment_service = _require_legacy_service("experiment_service", "Experiment service")
get_research_service = _require_legacy_service("research_service", "Research quest service")


def get_research_runtime_status(request: Request):
    """Return the sidecar status for diagnostics and feature discovery."""
    from .research_compat import get_research_runtime_status as resolve_status

    return resolve_status(request.app)
...
async def require_admin_user(request: Request, *, detail: str) -> None:
    ...
    # A few embedders (and the pre-DeerFlow Anaxa API) mount an individual
    # router on a bare FastAPI app without installing ``AuthMiddleware``.
    # Preserve that supported composition by falling back to the original
    # loopback/admin-token gate.  This does not weaken the production gateway:
    # ``create_app`` always installs AuthMiddleware, and any remote bare app
    # still has to present the explicit admin/proxy token.
    if getattr(request.state, "user", None) is None:
        app = getattr(request, "app", None)
        if app is not None and not getattr(app, "user_middleware", None):
