...
def _restore_raw_env_placeholders(
    server: McpServerConfigResponse,
    raw_server: Any,
) -> McpServerConfigResponse:
    """Restore safe ``$ENV_VAR`` references after masking resolved secrets."""
    ...
    for key, value in raw_env.items():
        if key in env and isinstance(value, str) and _ENV_PLACEHOLDER_RE.fullmatch(value):
            env[key] = value
    return server.model_copy(update={"env": env})


def _load_raw_mcp_servers() -> dict[str, Any]:
    """Load the unexpanded MCP server map for safe placeholder presentation."""
    ...
    return raw_servers if isinstance(raw_servers, dict) else {}
...
async def get_mcp_configuration(request: Request) -> McpConfigResponse:
    ...
    raw_servers = await asyncio.to_thread(_load_raw_mcp_servers)
    servers = {
        name: _restore_raw_env_placeholders(
            _mask_server_config(McpServerConfigResponse(**server.model_dump())),
            raw_servers.get(name),
        )
