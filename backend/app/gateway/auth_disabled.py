...
# ``MEDRIX_FLOW_ENV`` is the product-facing name used by the Gateway/Frontend
# deployment manifests and documentation. Keep the historical DeerFlow name
# and generic ``ENVIRONMENT`` alias for compatibility, but treat all three as
# authoritative production markers so auth-disabled test mode cannot leak into
# a production deployment configured with the documented variable.
_PRODUCTION_ENV_VARS: tuple[str, ...] = ("MEDRIX_FLOW_ENV", "DEER_FLOW_ENV", "ENVIRONMENT")
