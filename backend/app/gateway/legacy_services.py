...
class GatewayRunService:
    ...
    async def _execute_gateway_run(self, record: RunRecord, body: Any) -> None:
        ...
        try:
            ...
            if not started:
                return
            # Resolve these patch points through the public gateway module.
            # Anaxa integrations historically patched
            # ``app.gateway.services``; keeping that indirection lets the
            # compatibility service remain testable without coupling the
            # DeerFlow-native service module to the legacy client.
            import app.gateway.services as gateway_services

            gateway_services.get_paths().ensure_thread_dirs(record.thread_id)
            ...
            client = gateway_services.MedrixFlowClient(
                checkpointer=gateway_services.get_sync_checkpointer(),
