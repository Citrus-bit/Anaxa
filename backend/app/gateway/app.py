...
This gateway provides runtime endpoints for agent runs plus custom endpoints for models, MCP configuration, skills, and artifacts.
        ...
        openapi_tags=[
            ...
            {
                "name": "experiments",
                "description": "Structured experiment planning, execution, and artifact bundles",
            },
            {
                "name": "research",
                "description": "Research quest lifecycle, evidence gates, and manuscript workspace",
            },
            {
                "name": "models",
    ...
    # Include routers
    # Anaxa academic research API is mounted at /api/academic.  The backing
    # sidecar is optional and reports 503 until its legacy SQLite runtime is
    # available; mounting the routes unconditionally keeps the API contract
    # discoverable in OpenAPI and during gradual upgrades.
    app.include_router(academic.router)

    # Anaxa experiment API is mounted at /api/experiments.
    app.include_router(experiments.router)

    # Anaxa research quest API is mounted at /api/research.
    ...
    # Stateless Runs API (stream/wait without a pre-existing thread).  The
    # combined ``runs.router`` also carries Anaxa's hidden legacy thread-run
    # endpoints for embedded callers; production already mounts DeerFlow's
    # canonical thread-runs router above, so only mount the stateless surface.
    app.include_router(runs.stateless_router)
