...
_refresh_compose_cmd() {
    ...
    # Compose's service-level env_file only populates the container. Pass the
    # root file separately so ${...} expressions in the Compose model use the
    # same values. Commands run from DOCKER_DIR, hence the relative path.
    if [ -f "$PROJECT_ROOT/.env" ]; then
        env_file_arg=" --env-file ../.env"
    fi
    COMPOSE_CMD="${COMPOSE_BIN[*]}${env_file_arg} -p deer-flow-dev -f ${COMPOSE_FILE}"
}
_refresh_compose_cmd
...
start() {
    ...
    # start is the only command allowed to create these files. Refresh after
    # creation so Compose interpolation sees root .env before overlays are
