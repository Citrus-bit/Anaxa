...
if [ -f "$REPO_ROOT/.env" ]; then
    # A dotenv file is data, not a shell script. Parse only shell-compatible
    # names and quote values before evaluating generated export statements;
    # the backend loads the complete file, including names with dots.
    dotenv_exports="$("$(_pick_python)" - "$REPO_ROOT/.env" <<'PY'
import ast
import re
import shlex
import sys
...
# Production frontend builds and the Gateway proxy-auth check must use the
# exact same signing secret. Resolve it once for the whole launcher: an
# already-loaded value wins, then the persisted runtime secret, and finally a
# newly generated value that is stored for subsequent starts. The frontend
# command inherits this exported variable; embedding a fresh random value in
# the command would invalidate sessions and disagree with the Gateway on every
# restart.
_trim_value() {
    local value="$1"
...
resolve_better_auth_secret() {
    local configured secret_file="$DEER_FLOW_HOME/.better-auth-secret"

    configured="$(_trim_value "${BETTER_AUTH_SECRET:-}")"
    if [ -n "$configured" ]; then
        export BETTER_AUTH_SECRET="$configured"
        return 0
    fi

    if [ -f "$secret_file" ]; then
        configured="$(_trim_value "$(tr -d '\r\n' < "$secret_file")")"
        if [ -n "$configured" ]; then
            export BETTER_AUTH_SECRET="$configured"
            return 0
        fi
    ...
    if command -v "$DEERFLOW_PNPM_PYTHON" >/dev/null 2>&1 && \
        configured="$("$DEERFLOW_PNPM_PYTHON" -c 'import secrets; print(secrets.token_hex(32))' 2>/dev/null)"; then
        true
    elif command -v openssl >/dev/null 2>&1 && configured="$(openssl rand -hex 32 2>/dev/null)"; then
        true
    else
        echo "Cannot generate BETTER_AUTH_SECRET: Python and openssl are unavailable." >&2
        echo "Set BETTER_AUTH_SECRET before running --prod." >&2
        return 1
    fi
    ...
    echo "✓ BETTER_AUTH_SECRET generated → $secret_file"
}
...
# Frontend command. BETTER_AUTH_SECRET is exported above in production and is
# inherited by both `next build` and `next start`; keeping it out of this
# command string also avoids shell-quoting secrets supplied by the user.
if $DEV_MODE; then
    FRONTEND_CMD='env PORT=3000 "$DEERFLOW_PNPM_PYTHON" "$DEERFLOW_PNPM_RUNNER" run dev'
else
    # Keep the runner variables deferred until the nested service shell
    # executes them; this also preserves the upstream entrypoint contract.
    FRONTEND_CMD="env PORT=3000 \"\$DEERFLOW_PNPM_PYTHON\" \"\$DEERFLOW_PNPM_RUNNER\" run preview"
fi
...
# 1. Gateway API
run_service "Gateway" \
    "cd backend && env MEDRIX_FLOW_UI_PASSWORD= PYTHONPATH=. uv run --no-sync uvicorn app.gateway.app:app --host \"\${GATEWAY_HOST:-127.0.0.1}\" --port 8001 $GATEWAY_EXTRA_FLAGS > ../logs/gateway.log 2>&1" \
    8001 30
