...
if [ "$CMD" != "down" ] && [ -z "${BETTER_AUTH_SECRET:-}" ]; then
    _dotenv_better_auth_secret="$(read_dotenv_value BETTER_AUTH_SECRET)"
    if [ -n "$_dotenv_better_auth_secret" ]; then
        export BETTER_AUTH_SECRET="$_dotenv_better_auth_secret"
        echo -e "${GREEN}✓ BETTER_AUTH_SECRET loaded from $ENV_FILE${NC}"
    elif [ -f "$_secret_file" ]; then
...
if [ "$CMD" != "down" ] && [ -z "${DEER_FLOW_INTERNAL_AUTH_TOKEN:-}" ]; then
    _dotenv_internal_auth_token="$(read_dotenv_value DEER_FLOW_INTERNAL_AUTH_TOKEN)"
    if [ -n "$_dotenv_internal_auth_token" ]; then
        export DEER_FLOW_INTERNAL_AUTH_TOKEN="$_dotenv_internal_auth_token"
        echo -e "${GREEN}✓ DEER_FLOW_INTERNAL_AUTH_TOKEN loaded from $ENV_FILE${NC}"
    elif [ -f "$_internal_auth_token_file" ]; then
