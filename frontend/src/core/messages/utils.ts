...
export function groupMessages<T>(
  ...
  // `getMessageGroups` is the DeerFlow renderer and intentionally keeps a
  // leading orphan tool result visible while history pages are incomplete.
  // The original Anaxa helper had a stricter contract (orphan tools are
  // dropped and tool results are matched by call id), so retain that contract
  // for the standalone compatibility API used by integrations/tests.
  if (messages.length === 0) {
    return [];
  }
...
/**
 * Find the first visible human reply belonging to a clarification request.
 * A later clarification starts a new request, so an unanswered earlier one
 * must remain pending instead of consuming a response intended for it.
 */
