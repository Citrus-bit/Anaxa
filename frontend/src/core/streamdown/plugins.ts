...
/**
 * Backward-compatible preset used by the original Anaxa message renderer.
 * Streamdown 2.x owns word animation through `animated`; retaining the
 * historical export lets older consumers opt into the same behavior without
 * reintroducing the removed rehype word-splitting plugin.
 */
...
/** Convert authored raw HTML in a human message into literal text. */
type MarkdownNode = {
  type: string;
  value?: string;
...
export function remarkHumanHtmlAsText() {
  return (tree: MarkdownNode) => {
    const textify = (node: MarkdownNode) => {
      if (node.type === "html") {
        node.type = "text";
      }
      node.children?.forEach(textify);
    };
    textify(tree);
  };
}

// Human-authored markdown deliberately keeps URLs as plain text and does not
// parse raw HTML into elements. This is the legacy Anaxa surface; the shared
// sanitizer and KaTeX chain still apply to any resulting markdown nodes.
