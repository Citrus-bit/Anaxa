...
type InputBoxProps = Omit<ComponentProps<typeof PromptInput>, "onSubmit"> & {
  ...
  /** Deprecated alias for the original new-chat layout/experiment control. */
  isNewThread?: boolean;
  threadId: string;
  draftThreadId?: string;
  draftAgentName?: string | null;
  /** The active custom agent's configured default model, if any. */
...
/**
 * Public composer entry point. The `isNewThread` branch is kept as a small
 * legacy surface for integrations that mount the experiment toggle without
 * the full authenticated workspace provider tree.
 */
export function InputBox(props: InputBoxProps) {
  if (props.isNewThread && !props.isWelcomeMode) {
    return <LegacyNewThreadInputBox {...props} />;
  }
  return <InputBoxContent {...props} />;
}
      ...
      {isNewThread &&
        ...
        !selectedSlashSkill && (
          <SyntheticExperimentModeToggle
            checked={Boolean(
              (context as { synthetic_data_mode?: unknown }).synthetic_data_mode,
            )}
...
function SyntheticExperimentModeToggle({
  ...
  const { t } = useI18n();
  const inputBox = t.inputBox as typeof t.inputBox & {
    syntheticExperimentMode?: string;
    syntheticExperimentModeDescription?: string;
  };
  const label = inputBox.syntheticExperimentMode ?? "Simulation mode";
  const description =
    inputBox.syntheticExperimentModeDescription ??
    "Allow simulated personal experiment data";
...
function LegacyNewThreadInputBox({
  className,
  context,
  onContextChange,
}: InputBoxProps) {
