...
The chart also supports Anaxa's optional outer UI-password gate. The password
and admin bypass token are written to the app Secret (or read from the Secret
named by `existingAppSecret`). The UI password is injected into Frontend; the
shared Better Auth secret and optional admin token are also injected into
Gateway so nginx's proxy authorization can be verified inside the cluster:

```yaml
frontend:
  auth:
    environment: production
    uiPassword: "replace-with-a-long-random-password"
    # Optional for scripted administration through nginx.
    adminToken: "replace-with-a-separate-random-token"
```

The chart keeps `NODE_ENV=production` and defaults `frontend.auth.environment`
to `production`. Therefore a rendered/deployed chart remains installable with
an empty password, but protected pages and API routes fail closed until an
operator supplies `frontend.auth.uiPassword` (or the matching Secret key).
`/health` and `/nginx-health` stay public for Kubernetes probes. For an
externally managed Secret, set `existingAppSecret` and include
`BETTER_AUTH_SECRET`, `DEER_FLOW_INTERNAL_AUTH_TOKEN`, and optionally
`MEDRIX_FLOW_UI_PASSWORD` / `MEDRIX_GATEWAY_ADMIN_TOKEN`.
